...
let getallen = [];
for (let i = 0; i < 100; i++) {
    let getal = parseInt(prompt("Geef een getal in (negatief om te stoppen):"));
    if (getal < 0) {
        break;
    }
    getallen.push(getal);
}
console.log("De ingevoerde getallen zijn:", getallen);

let hoeveelstegetal = parseInt(prompt("Welk getal wilt u zien? Voer het getal in:"));

let index = getallen.findIndex(getal => getal === hoeveelstegetal);
console.log(`het getal ${hoeveelstegetal} is het getal ${index} `)
